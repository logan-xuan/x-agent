...
4. **Given** 身份初始化已完成，**When** 用户再次启动系统，**Then** 系统跳过初始化直接加载已有身份
...
2. **Given** 用户与 AI 进行对话，**When** 对话产生重要内容（用户标记或 AI 识别），**Then** 内容被追加到当日日志
...
- 记忆文件损坏时，系统如何恢复？
- 用户身份信息变更（如姓名、职业改变），如何更新 OWNER.md？
- 向量数据库存储空间不足时如何处理？
...
- **Memory Entry（记忆条目）**: 单条记忆记录，包含内容、时间戳、类型（对话/决策/摘要）和向量嵌入
