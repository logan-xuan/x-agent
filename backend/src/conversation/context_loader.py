...
class ContextLoader:
    ...
    def __init__(self, workspace_path: str = "workspace") -> None:
        ...
        logger.info("ContextLoader initialized", extra={"workspace_path": str(self.workspace_path)})
    ...
    def load_context(
        ...
        if session_type == SessionType.SHARED:
            ...
            logger.info("MEMORY.md excluded for shared context", extra={"session_id": session_id})
    ...
    def load_agents_content(self, force_reload: bool = False) -> tuple[str, bool]:
        ...
        if (
            not force_reload
            and self._agents_content is not None
            and current_mtime == self._agents_mtime
        ):
        ...
        try:
            ...
            logger.error("Failed to load AGENTS.md", extra={"error": str(error)})
    ...
    def reload_agents_if_changed(self) -> tuple[str, bool, float]:
        ...
        if was_reloaded:
            logger.info("AGENTS.md reloaded", extra={"reload_time_ms": reload_time_ms})
