...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # 9. Initialize and start Channel adapters (DingTalk, Feishu, etc.)
    from .channel.registry import (
    ...
    # Create dispatcher factory for channel adapters
    def _create_dispatcher():
        """Create a new GatewayDispatcher instance."""
        return GatewayDispatcher()

    # Register channel adapters from config
    registered_count = 0
    for channel_config in config.multi_agent.channels:
        adapter = create_channel_adapter(channel_config, _create_dispatcher)
        if adapter:
            try:
                channel_registry.register(channel_config.id, adapter)
                registered_count += 1
            except ValueError as e:
                logger.warning(
                    "Failed to register channel adapter",
                    extra={"channel_id": channel_config.id, "error": str(e)},
                )
    ...
    if registered_count > 0:
        ...
        logger.info(
            ...
            extra={"count": registered_count},
        )
    else:
        logger.info("No channel adapters to start")

    app.state.channel_registry = channel_registry

    # 10. Install global exception hooks
    ...
    if hasattr(app.state, "channel_registry") and app.state.channel_registry:
        await app.state.channel_registry.stop_all()
