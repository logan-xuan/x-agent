...
# Helper function to generate problem guidance data
def generate_problem_guidance_for_frontend(
    ...
    Args:
        error_type: Type of error (e.g., 'tool_not_found')
        error_message: Error message text
        context: Additional context information
        
    Returns:
        Dictionary suitable for ProblemGuidanceCard component
    """
    try:
        ...
        visualization = report.to_visualization()
        
        # Debug logging
        from ..utils.logger import get_logger
        logger = get_logger(__name__)
        logger.debug(
            "Generated problem guidance",
            extra={
                "error_type": error_type,
                "guidance_type": visualization.get("type"),
                "severity": visualization.get("severity"),
            }
        )
        ...
        logger = get_logger(__name__)
        logger.error(
            f"Failed to generate problem guidance: {type(e).__name__}: {e}",
            extra={
                "error_type": error_type,
        ...
        return {
            "type": str(error_type) if error_type else "unknown_error",
            "severity": "high",
            "title": "执行错误",
            "description": str(error_message) if error_message else "发生未知错误",
            "context": context,
            "steps": [],
            "auto_fixes": ["请检查配置和依赖项", "查看日志了解详细信息"],
            "user_info_needed": ["请提供更多上下文信息"],
        }
