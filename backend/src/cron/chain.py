...
class JobChain:
    ...
    def __post_init__(self):
        ...
        for step in self.steps:
            if step.on_success and step.on_success not in step_ids:
                raise ValueError(
                    f"Step {step.step_id} references unknown on_success: {step.on_success}"
                )
            if step.on_failure and step.on_failure not in step_ids:
                raise ValueError(
                    f"Step {step.step_id} references unknown on_failure: {step.on_failure}"
                )
    ...
    def get_next_step(
        self, current_step_id: str, result: Any, success: bool
    ) -> JobChainStep | None:
