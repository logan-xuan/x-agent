...
try:
    ...
    from .skill_parser import SkillParseError, SkillParser
except ImportError:  # 兼容 tests 直接把 src/ 加到 sys.path 顶层导入 services.*
    from models.skill import SkillMetadata  # type: ignore
    from services.skill_parser import SkillParseError, SkillParser  # type: ignore
...
class SkillRegistry:
    ...
    def reload_if_changed(self) -> bool:
        ...
        """
        logger.warning("reload_if_changed() is deprecated, use clear_cache() instead")
        ...
        logger.info(
            f"Skills reloaded: {old_count} → {new_count}", extra={"added": new_count - old_count}
        )
    ...
    def _discover_all_skills(self) -> list[SkillMetadata]:
        ...
        if user_skills_path.exists():
            try:
                ...
                logger.info(
                    f"Found {len(user_skills)} user skills", extra={"path": str(user_skills_path)}
                )
            except Exception as e:
    ...
    def _scan_directory(self, directory: Path, level: str = "unknown") -> list[SkillMetadata]:
