...
class FlowBuilder:
    ...
    def _filter_timeline(
        ...
        for event in timeline:
            ...
            if detail_level == "medium" and any(
                keyword in module for keyword in ["middleware", "memory", "service"]
            ):
                filtered.append(event)
                continue
            ...
            if any(
                keyword in event_name for keyword in ["started", "completed", "error", "failed"]
            ):
                filtered.append(event)
                continue
    ...
    def _build_nodes(
        ...
        for idx, event in enumerate(timeline):
            ...
            node_key = f"{module}" if detail_level == "high" else f"{module}-{idx}"
            ...
            if operation_type:
                if detail_level == "high":
                    ...
                    label = self._get_detailed_operation_label(
                        operation_type, event_data, event_name
                    )
            else:
                if detail_level == "high":
    ...
    def _get_detailed_operation_label(
        self, operation_type: str, event_data: dict, event_name: str
    ) -> str:
        ...
        if operation_type == "tool_call":
            ...
            return (
                f"Plan: {plan_action[:30]}..."
                if plan_action and len(plan_action) > 30
                else f"Plan: {plan_action}"
            )
        else:
