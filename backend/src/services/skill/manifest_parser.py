...
class ManifestParser:
    ...
    def __init__(self, schema_path: Path | None = None) -> None:
        ...
        logger.debug(
            "ManifestParser initialized",
            extra={"schema_loaded": self._schema is not None}
        )
    ...
    def _parse_manifest_json(self, manifest_path: Path) -> dict[str, Any]:
        ...
        if self._schema_validator:
            ...
            if errors:
                ...
                raise ManifestParseError(
                    f"Schema validation failed for {field}: {error.message}"
                )
    ...
    def _parse_skill_md(self, skill_md_path: Path) -> dict[str, Any]:
        ...
        if len(parts) < 3:
            raise ManifestParseError(
                f"Invalid YAML frontmatter format: {skill_md_path}"
            )
        ...
        if not isinstance(frontmatter, dict):
            raise ManifestParseError(
                f"YAML frontmatter must be a mapping: {skill_md_path}"
            )
        
        # 校验必需字段
        if "name" not in frontmatter:
            raise ManifestParseError(f"SKILL.md must contain 'name' field")
        if "description" not in frontmatter:
            raise ManifestParseError(f"SKILL.md must contain 'description' field")
    ...
    def _convert_skill_md_to_manifest(
        ...
        direct_fields = [
            "vendor", "tags", "domains", "examples", "keywords", "priority",
            "emoji", "homepage", "license", "context",
        ]
