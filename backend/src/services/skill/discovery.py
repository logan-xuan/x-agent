...
class SkillDiscovery:
    ...
    def discover(
        ...
        for skill in all_skills:
            ...
            # 计算语义相似度
            semantic_sim = self.embedder.cosine_similarity(query_embedding, skill_embedding)
    ...
    def _create_skill_card(
        ...
        """从技能和评分创建 SkillCard。"""
        # 获取缺失参数
        missing_params = self.scorer.get_missing_params(skill, context.available_params)
        ...
        # 判断是否需要审批
        approval_required = (
            skill.risk_level.value in ["high", "critical"] or skill.approval_mode.value != "auto"
        )
