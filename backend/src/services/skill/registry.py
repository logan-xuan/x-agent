...
from .manifest_parser import ManifestParser, ManifestParseError
...
class SkillRegistry:
    ...
    def __init__(
        ...
        logger.info(
            ...
            extra={
                ...
                "system_skills_dir": str(self.system_skills_dir) if self.system_skills_dir else None,
    ...
    def list_skills(
        ...
        return [
            manifest
            for manifest, skill_source in self._cache.values()
    ...
    def get_skill_with_source(
        self, skill_id: str
    ) -> tuple[SkillManifest, SkillSource] | None:
        """获取技能及其来源。
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        user_count = sum(
            1 for _, source in self._cache.values()
    ...
    def _reload_cache(self) -> None:
        ...
        if self.system_skills_dir and self.system_skills_dir.exists():
            system_skills = self._scan_directory(
                self.system_skills_dir, SkillSource.SYSTEM
            )
        ...
        if self.user_skills_dir and self.user_skills_dir.exists():
            user_skills = self._scan_directory(
            ...
            logger.debug(
                f"已加载 {len(user_skills)} 个用户技能",
                extra={"path": str(self.user_skills_dir)}
            )
    ...
    def _scan_directory(
        ...
        for item in directory.iterdir():
            ...
            try:
                ...
                logger.warning(
                    f"解析技能 {item.name} 失败: {e}",
                    extra={"path": str(item)}
                )
            except Exception as e:
                logger.error(
                    f"解析技能 {item.name} 时发生意外错误: {e}",
                    extra={"path": str(item)}
                )
...
def get_skill_registry() -> SkillRegistry:
    ...
    if _registry is None:
        raise RuntimeError(
            "Skill registry not initialized. Call init_skill_registry() first."
        )
    return _registry
