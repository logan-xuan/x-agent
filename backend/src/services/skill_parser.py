...
class SkillParser:
    ...
    def parse(self, skill_md_path: Path) -> SkillMetadata:
        ...
        try:
            ...
            metadata = SkillMetadata(
                ...
                keywords=metadata_dict.get(
                    "keywords", []
                ),  # 🔥 NEW: Read keywords for skill matching
                auto_trigger=metadata_dict.get(
                    "auto-trigger", True
                ),  # 🔥 NEW: Support auto-trigger config
                ...
                allowed_tools=metadata_dict.get(
                    "allowed_tools"
                ),  # Use underscore to match SKILL.md format
    ...
    def _parse_frontmatter(self, content: str, file_path: Path) -> dict[str, Any]:
        ...
        if not content.strip().startswith("---"):
            raise SkillParseError(f"SKILL.md must start with YAML frontmatter (---): {file_path}")
        ...
        if metadata is None:
            raise SkillParseError(f"YAML frontmatter is empty or invalid: {file_path}")
        ...
        if "name" not in metadata:
            raise SkillParseError(f"SKILL.md must have 'name' field: {file_path}")
        if "description" not in metadata:
            raise SkillParseError(f"SKILL.md must have 'description' field: {file_path}")
