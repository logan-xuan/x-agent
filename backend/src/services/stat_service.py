...
class StatService:
    ...
    async def get_recent_errors(
        ...
        async with self._storage.session() as db_session:
            query = (
                select(LLMRequestStat)
                .where(LLMRequestStat.success == 0)
                .order_by(LLMRequestStat.created_at.desc())
                .limit(limit)
            )
    ...
    async def get_daily_stats(
        ...
        async with self._storage.session() as db_session:
            ...
            query = (
                ...
                .where(LLMRequestStat.created_at >= start_time)
                .group_by(func.date(LLMRequestStat.created_at))
                .order_by(func.date(LLMRequestStat.created_at).desc())
            )
