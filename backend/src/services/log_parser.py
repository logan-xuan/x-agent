...
class PromptLogEntry:
    ...
    def __init__(self, raw_data: dict[str, Any]) -> None:
        ...
        self.call_id = raw_data.get("call_id")
...
class LogParser:
    ...
    def parse_x_agent_logs(self, trace_id: str) -> list[LogEntry]:
        ...
        try:
            with open(self.x_agent_log, encoding="utf-8") as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
                    ...
                    try:
                        ...
                        if "message" in data and isinstance(data["message"], str):
                            try:
                                ...
                                if "extra" in message_data and isinstance(
                                    message_data["extra"], dict
                                ):
    ...
    def parse_prompt_llm_logs(self, trace_id: str) -> list[PromptLogEntry]:
        ...
        try:
            with open(self.prompt_llm_log, encoding="utf-8") as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
    ...
    def build_timeline(self, trace_id: str) -> dict[str, Any]:
            ...
            timeline.append(
                {
                    ...
                    "data": {
                        ...
                        "request_preview": self._preview_messages(
                            entry.request.get("messages", [])
                        ),
        ...
        valid_timestamps = [e["timestamp"] for e in timeline if e.get("timestamp")]
    ...
    def _enrich_timeline_data(self, timeline: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for event in timeline:
            ...
            # Detect tool calls
            if (
                ("tool" in module and "call" in event_name)
                or any(key in data for key in ["tool_name", "tool_call", "tool_args"])
                or "run_tool" in event_name
                or "execute_tool" in event_name
            ):
            ...
            # Detect skill calls
                ...
                ("skill" in module and "call" in event_name)
                or "skill_call" in event_name
                or "execute_skill" in event_name
                or data.get("skill_name")
            ):
                enhanced_data["operation_type"] = "skill_call"
                enhanced_data["skill_name"] = (
                    data.get("skill_name") or data.get("name") or "unknown"
                )
            ...
            # Detect command execution
                ...
                ("command" in event_name)
                or ("terminal" in module)
                or any(key in data for key in ["command", "cmd", "shell_command"])
                or "execute" in event_name
            ):
                enhanced_data["operation_type"] = "command"
                enhanced_data["command"] = (
                    data.get("command") or data.get("cmd") or data.get("shell_command")
                )
            ...
            # Detect memory operations
                ...
                (
                    "memory" in module
                    and any(op in event_name for op in ["store", "save", "create", "update"])
                )
                or any(key in data for key in ["memory_store", "memory_save", "memory_create"])
                or ("memory" in module and "write" in event_name)
            ):
                ...
                ("memory" in module and "query" in event_name)
                or ("memory" in module and "search" in event_name)
                or any(key in data for key in ["memory_query", "memory_search", "memory_get"])
                or ("vector_store" in module)
            ):
            ...
            # Detect ReAct loop operations
                ...
                keyword in event_name for keyword in ["react", "think", "observe", "act", "reason"]
            ) or any(keyword in module for keyword in ["react", "reasoning"]):
            ...
            # Detect Plan Mode operations
                ...
                for keyword in ["plan", "planning", "generate_plan", "execute_plan"]
            ) or any(keyword in module for keyword in ["plan", "planner"]):
