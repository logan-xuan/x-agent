...
class AliyunWebSearchTool(BaseTool):
    ...
    async def execute(
        self, query: str, max_results: int = 5, content_type: str = "snippet"
    ) -> ToolResult:
        ...
        try:
            ...
            logger.error(
                "Aliyun web search failed", extra={"query": query, "error": str(e)}, exc_info=True
            )
        ...
        try:
            ...
            if "result" in response_data and "search_result" in response_data["result"]:
                ...
                if "usage" in response_data:
                    ...
                    total_tokens = usage.get("rewrite_model.total_tokens", 0) + usage.get(
                        "filter_model.total_tokens", 0
                    )
                    ...
                    logger.debug("Token usage tracked", extra=self.last_usage_info)
            ...
            logger.error(f"URL Error: {e.reason}", extra={"query": query}, exc_info=True)
            results.append(
                {
            ...
            logger.error(f"Unexpected error: {str(e)}", extra={"query": query}, exc_info=True)
            results.append(
                {
                    "title": "Search Error",
