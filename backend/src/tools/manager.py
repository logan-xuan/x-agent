...
class ToolManager:
    ...
    def _correct_tool_parameters(self, tool_name: str, arguments: dict) -> dict:
        ...
        if tool_name == "search_files" and "search_dir" in corrected:
            ...
            logger.debug(
                "Fixed search_files parameter: search_dir -> path", extra={"tool_name": tool_name}
            )
    ...
    def register(self, tool: BaseTool) -> None:
        ...
        if tool.name in self._tools:
            logger.warning("Tool already registered, replacing", extra={"tool_name": tool.name})
    ...
    async def execute(
        ...
        if (
            skill_context
            and hasattr(skill_context, "allowed_tools")
            and skill_context.allowed_tools
        ) and name not in skill_context.allowed_tools:
            error_msg = (
        ...
        if not is_valid:
            logger.warning(
                "Tool parameter validation failed", extra={"tool_name": name, "error": error}
            )
