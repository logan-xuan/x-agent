...
class SpiritLoader:
    ...
    def __init__(self, workspace_path: str | None = None) -> None:
        ...
        logger.info("SpiritLoader initialized", extra={"workspace_path": self.workspace_path})
    ...
    def get_identity_status(self) -> dict[str, bool]:
        ...
        logger.debug("Identity status checked", extra=status)
    ...
    def initialize_identity(
        ...
        """
        logger.info("Initializing identity", extra={"owner_name": owner_name})
    ...
    def update_spirit(self, config: SpiritConfig) -> bool:
        ...
        if success:
            ...
            logger.info(
                "Spirit config updated", extra={"role": config.role[:50] if config.role else ""}
            )
