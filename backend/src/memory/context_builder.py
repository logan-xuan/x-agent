...
class ContextBuilder:
    ...
    def __init__(self, workspace_path: str | None = None) -> None:
        ...
        logger.info("ContextBuilder initialized", extra={"workspace_path": self.workspace_path})
    ...
    def _load_tools(self) -> list[ToolDefinition]:
        ...
        logger.debug("Tools loaded", extra={"count": len(tools)})
    ...
    def _load_recent_logs(self, days: int = 7) -> list[DailyLog]:
        ...
        logger.debug("Recent logs loaded", extra={"count": len(logs), "days": days})
    ...
    def _load_identity(self) -> IdentityConfig | None:
        ...
        try:
            ...
            logger.error("Failed to load IDENTITY.md", extra={"error": str(e)})
            return None
    ...
    def _load_long_term_memory(self) -> str:
        ...
        try:
            ...
            logger.error("Failed to load long-term memory", extra={"error": str(e)})
            return ""
    ...
    def format_context_for_prompt(self, context: ContextBundle) -> str:
        ...
        logger.debug("Context formatted for prompt", extra={"prompt_length": len(prompt)})
