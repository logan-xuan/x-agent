...
class VectorStore:
    ...
    def _load_vss_extension(self, conn: Any) -> bool:
        ...
        try:
            ...
            if os.path.exists(vector0_path) and os.path.exists(vss0_path):
                ...
                logger.warning("sqlite-vss extension files not found", extra={"vss_dir": vss_dir})
                return False

        except Exception as e:
            logger.warning("Failed to load sqlite-vss extension", extra={"error": str(e)})
            return False
        ...
        try:
            ...
            if self._vss_available:
                try:
                    ...
                    self._vss_table_exists = False
                    logger.warning("Failed to create vss virtual table", extra={"error": str(e)})
            else:
                self._vss_table_exists = False
                logger.warning(
            ...
            logger.error("Failed to initialize VectorStore", extra={"error": str(e)})
            raise
    ...
    def insert(
        ...
        try:
            ...
            """,
                (entry_id, content, content_type, "", json.dumps(metadata) if metadata else None),
            )
            ...
            conn.commit()
            logger.debug(
                "Vector entry inserted", extra={"entry_id": entry_id, "content_type": content_type}
            )

        except Exception as e:
            logger.error("Failed to insert vector", extra={"entry_id": entry_id, "error": str(e)})
            raise
    ...
    def delete(self, entry_id: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to delete vector", extra={"entry_id": entry_id, "error": str(e)})
            return False
    ...
    def get_embedding(self, entry_id: str) -> list[float] | None:
        ...
        try:
            ...
            logger.error("Failed to get embedding", extra={"entry_id": entry_id, "error": str(e)})
            return None
    ...
    def get_entry(self, entry_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error("Failed to get entry", extra={"entry_id": entry_id, "error": str(e)})
            return None
    ...
    def count(self) -> int:
        ...
        try:
            ...
            logger.error("Failed to count entries", extra={"error": str(e)})
            return 0
    ...
    def clear(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to clear VectorStore", extra={"error": str(e)})
            raise
    ...
    def get_all_entries(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to get all entries", extra={"error": str(e)})
            return []
