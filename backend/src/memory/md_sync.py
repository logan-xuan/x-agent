...
class MarkdownSync:
    ...
    def __init__(self, workspace_path: str) -> None:
        ...
        logger.info("MarkdownSync initialized", extra={"workspace_path": str(self.workspace_path)})
    ...
    def load_spirit(self) -> SpiritConfig | None:
        ...
        try:
            with open(self.spirit_path, encoding="utf-8") as f:
                content = f.read()
            ...
            config.last_modified = datetime.fromtimestamp(self.spirit_path.stat().st_mtime)
            ...
            logger.error("Failed to load SPIRIT.md", extra={"error": str(e)})
            return None
...
"""
            ...
            logger.info("SPIRIT.md saved", extra={"role": config.role[:50] if config.role else ""})
            return True

        except Exception as e:
            logger.error("Failed to save SPIRIT.md", extra={"error": str(e)})
            return False
    ...
    def load_owner(self) -> OwnerProfile | None:
        ...
        try:
            ...
            config.last_modified = datetime.fromtimestamp(self.owner_path.stat().st_mtime)

            logger.info("OWNER.md loaded", extra={"name": config.name})
            return config

        except Exception as e:
            logger.error("Failed to load OWNER.md", extra={"error": str(e)})
            return None
...
"""
            ...
            logger.info("OWNER.md saved", extra={"name": config.name})
            return True

        except Exception as e:
            logger.error("Failed to save OWNER.md", extra={"error": str(e)})
            return False
    ...
    def load_tools(self) -> list[ToolDefinition]:
        ...
        try:
            ...
            logger.info("TOOLS.md loaded", extra={"tools_count": len(tools)})
            return tools

        except Exception as e:
            logger.error("Failed to load TOOLS.md", extra={"error": str(e)})
            return []
    ...
    def load_daily_log(self, date: str) -> DailyLog | None:
        ...
        try:
            ...
            logger.error("Failed to load daily log", extra={"date": date, "error": str(e)})
            return None
    ...
    def append_memory_entry(self, entry: MemoryEntry) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to append memory entry", extra={"entry_id": entry.id, "error": str(e)}
            )
            return False
    ...
    def load_entries_from_log(self, date: str) -> list[MemoryEntry]:
        ...
        try:
            ...
            logger.error("Failed to load entries from log", extra={"date": date, "error": str(e)})
            return entries
    ...
    def list_all_entries(
        ...
        log_files = sorted(self.memory_path.glob("*.md"), key=lambda p: p.stem, reverse=True)
    ...
    def delete_entry(self, entry_id: str) -> bool:
        ...
        try:
            ...
            logger.info("Memory entry deleted", extra={"entry_id": entry_id, "date": date})
            return True

        except Exception as e:
            logger.error(
                "Failed to delete memory entry", extra={"entry_id": entry_id, "error": str(e)}
            )
            return False
    ...
    def sync_all_entries_to_vector_store(
        ...
        logger.info("Bulk sync completed", extra={"total_entries": len(entries), "synced": synced})
    ...
    def delete_entry_from_vector_store(
        ...
        try:
            ...
            if deleted:
                logger.info("Entry deleted from vector store", extra={"entry_id": entry_id})
            return deleted
    ...
    def recover_entries_from_vector_store(
        ...
        try:
            ...
            logger.error("Failed to recover entries from vector store", extra={"error": str(e)})
            return 0
    ...
    def sync_on_file_change(
        ...
        try:
            ...
            logger.error(
                "Failed to sync file change", extra={"file_path": file_path, "error": str(e)}
            )
            return False
