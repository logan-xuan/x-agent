...
class TimedSizeRotatingFileHandler(logging.Handler):
    ...
    def open(self):
        ...
        if self.stream is None:
            self.stream = open(self.base_filename, "a", encoding=self.encoding)  # noqa: SIM115
        ...
        while True:
            ...
            if len(name_parts) == 2:
                new_filename = os.path.join(
                    dir_name, f"{name_parts[0]}-{date_suffix}-{seq:02d}{name_parts[1]}"
                )
            else:
                new_filename = os.path.join(dir_name, f"{base_name}-{date_suffix}-{seq:02d}")
...
class LLMFallbackEventLogger:
    """Dedicated logger for provider fallback events.

    Writes a single structured event for each provider switch so operators can
    grep one file instead of reconstructing fallback chains from general logs.
    """

    _instance: "LLMFallbackEventLogger | None" = None
    _log_file: Path | None = None
    _file_handler: TimedSizeRotatingFileHandler | None = None
    _initialized: bool = False

    def __new__(cls) -> "LLMFallbackEventLogger":
        if cls._instance is None:
            cls._instance = super().__new__(cls)
        return cls._instance
    ...
    def initialize(
        ...
        """Initialize the fallback event logger."""
        if self._initialized:
            return

        self._log_file = Path(log_dir) / "llm-fallback-event.log"
        self._log_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    def log_event(self, **payload: Any) -> None:
        """Write one fallback event entry to the dedicated log file."""
        if not self._initialized:
            self.initialize()
        ...
        log_record.request_id = payload.get("request_id")
        log_record.session_id = payload.get("session_id")
        self._file_handler.emit(log_record)


_llm_fallback_event_logger = LLMFallbackEventLogger()
...
def get_llm_fallback_event_logger(
    config: LoggingConfig | None = None,
) -> LLMFallbackEventLogger:
    """Get the global LLM fallback event logger instance."""
    if not _llm_fallback_event_logger._initialized:
        if config:
            _llm_fallback_event_logger.initialize(
                log_dir=str(Path(config.file).parent),
                max_size=config.prompt_llm_max_size,
                backup_count=config.prompt_llm_backup_count,
                when=config.when,
                interval=config.interval,
            ...
            _llm_fallback_event_logger.initialize()
    return _llm_fallback_event_logger
