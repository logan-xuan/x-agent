...
def validate_path_in_workspace(
    file_path: str | Path, workspace_path: str | Path = "workspace"
) -> Path:
    ...
    if target.is_symlink():
        ...
        try:
            ...
            raise PathValidationError(f"Symlink '{file_path}' points outside workspace") from exc

    return target


async def async_read_file(
    file_path: str | Path, encoding: str = "utf-8", max_size_mb: float = 1.0
) -> tuple[str, bool]:
    ...
    try:
        ...
        if size_mb > max_size_mb:
            logger.warning(f"File too large: {path} ({size_mb:.2f}MB > {max_size_mb}MB)")
            return "", False
...
class AsyncFileLock:
    ...
    def __init__(self, file_path: str | Path, timeout: float = 10.0) -> None:
    ...
    def __enter__(self) -> "AsyncFileLock":
        ...
        try:
            ...
            if (
                ...
                or FileLockTimeout is not None
                and isinstance(exc, type(FileLockTimeout))
            ):
...
def get_daily_memory_path(
    date: datetime | str | None = None, workspace_path: str | Path = "workspace"
) -> Path:
...
def get_yesterday_memory_path(workspace_path: str | Path = "workspace") -> Path:
