...
class InMemoryRateLimiter:
    ...
    async def check_rate_limit(self, request: Request) -> float | None:
        ...
        async with self._lock:
            ...
            state.burst_timestamps = [
                ts
                for ts in state.burst_timestamps
    ...
    def cleanup_old_clients(self, max_age_hours: int = 24) -> None:
        ...
        to_remove = [key for key, state in self._clients.items() if state.last_reset < cutoff]
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if retry_after is not None:
            ...
            return Response(
                content=(
                    '{"success":false,"error":{"code":"ERR_RATE_LIMITED","message":"Rate limit exceeded",'
                    f'"details":{{"retry_after":{int(retry_after)}}}}}'
                ),
                status_code=429,
                headers={
