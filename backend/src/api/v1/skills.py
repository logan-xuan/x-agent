    ...
    try:
        ...
        if source:
            ...
            if source_lower == "user":
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid source: {source}. Use 'user' or 'system'."
                )
        ...
        logger.error(f"Failed to list skills: {e}", extra={"error": str(e)}, exc_info=True)
        return []
...
async def clear_cache() -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Failed to clear cache: {exc}", exc_info=True)
...
async def get_stats() -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Failed to get stats: {exc}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get stats: {str(exc)}") from exc
...
async def discover_skills(
    ...
    try:
        ...
        logger.error(f"Failed to discover skills: {exc}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to discover skills: {str(exc)}",
        ) from exc
...
async def get_skill(skill_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Failed to get skill: {exc}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get skill: {str(exc)}") from exc
...
async def invoke_skill(
    ...
    try:
        ...
        logger.error(f"Failed to invoke skill: {exc}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to invoke skill: {str(exc)}",
        ) from exc
