...
async def get_identity_status() -> IdentityStatus:
    ...
    logger.info("Identity status checked", extra={"status": status})
...
async def initialize_identity(request: IdentityInitRequest) -> IdentityInitResponse:
    ...
    logger.info("Initializing identity", extra={"owner_name": request.owner_name})
    ...
    try:
        ...
        logger.info("Identity initialized successfully", extra={"owner_name": owner.name})
        ...
        logger.error("Failed to initialize identity", extra={"error": str(e)})
...
async def update_spirit(config: SpiritConfig) -> SpiritConfig:
    ...
    logger.info("Spirit config updated", extra={"role": config.role[:50] if config.role else ""})
    return config
...
class SearchRequestModel(BaseModel):
    ...
    limit: int | None = Field(
        default=None, ge=1, le=100, description="Maximum number of results (default from config)"
    )
    offset: int = Field(default=0, ge=0, description="Pagination offset")
    content_type: str | None = Field(default=None, description="Filter by content type")
    min_score: float | None = Field(
        default=None, ge=0.0, le=1.0, description="Minimum relevance score (default from config)"
    )
