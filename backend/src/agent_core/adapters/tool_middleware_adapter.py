...
class BudgetControlMiddleware(ToolMiddleware):
    ...
    async def before_execute(self, ctx: ToolCallContext) -> ToolCallContext:
        ...
        if self.max_tool_calls_total > 0 and current_total >= self.max_tool_calls_total:
            return self._abort(
                ctx, state, scope_id, f"tool budget exhausted ({self.max_tool_calls_total})"
            )
        ...
        ctx.metadata["tool_budget_max_total_calls"] = self.max_tool_calls_total
        ctx.metadata["tool_budget_max_calls_by_name"] = dict(self.max_tool_calls_by_name)
        return ctx
        ...
        ctx.metadata.update(
            {
                ...
                "tool_budget_max_total_calls": self.max_tool_calls_total,
                "tool_budget_max_calls_by_name": dict(self.max_tool_calls_by_name),
            }
        )
        logger.warning(
    ...
    def _prune_states(self) -> None:
        ...
        stale = [
            scope_id for scope_id, state in self._states.items() if state.last_seen_at < cutoff
        ]
