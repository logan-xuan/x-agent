...
class HookPoint(Enum):
    ...
    BEFORE_TOOL_SELECTION = (
        "before_tool_selection"  # LLM 返回工具调用后、执行前，可修改/过滤工具列表
    )
...
class HookRegistry:
    ...
    def __init__(self):
        """初始化注册中心."""
        self._hooks: dict[HookPoint, list[HookEntry]] = {point: [] for point in HookPoint}
    ...
    def unregister(self, point: HookPoint, name: str) -> bool:
        ...
        for i, entry in enumerate(entries):
            if entry.name == name:
                entries.pop(i)
                logger.debug("Hook unregistered", extra={"point": point.value, "name": name})
                return True
        return False
