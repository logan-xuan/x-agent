...
describe('AgentMessageItem', () => {
  it('renders markdown image syntax as an inline image for assistant messages', () => {
    render(
      <AgentMessageItem
        message={buildAssistantMessage('先看这张图\n\n![小猫](http://localhost:8888/cat.png)')}
      />,
    )
  ...
  it('renders transcript and audio player metadata when voice fields are present', () => {
    const message: AgentMessage = {
      ...
      audio: {
        publicUrl: 'http://localhost:8888/api/v1/assets/audio/main-agent/2026-04-12/reply.mp3',
        provider: 'edge',
    ...
    expect(screen.getByText('edge · zh-CN-YunxiNeural · mp3')).toBeInTheDocument()
