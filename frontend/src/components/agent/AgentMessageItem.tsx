...
function renderAssistantInlineText(
  content: string,
  segmentKey: string,
): ReactNode[] {
...
function renderAssistantText(content: string, segmentKey: string): ReactNode[] {
  ...
  const flushParagraph = () => {
    ...
    blocks.push(
      <span
        key={`${segmentKey}-paragraph-${blocks.length}`}
        className="whitespace-pre-wrap"
      >
        {renderAssistantInlineText(
          paragraph,
          `${segmentKey}-paragraph-${blocks.length}`,
        )}
      </span>,
    );
  ...
  const flushList = () => {
    ...
    blocks.push(
      <ul
        key={`${segmentKey}-list-${blocks.length}`}
        className="my-2 list-disc space-y-1 pl-5"
      >
        {listItems.map((item, index) => (
          <li key={`${segmentKey}-list-item-${index}`}>
            {renderAssistantInlineText(
              item,
              `${segmentKey}-list-item-${index}`,
            )}
          </li>
        ))}
      </ul>,
  ...
  return blocks.length > 0
    ? blocks
    : renderAssistantInlineText(content, segmentKey);
}
  ...
  return (
    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-500/25">
      ...
      >
        ...
        <path
          d="M12 5v4M12 15v4M5 12h4M15 12h4M7 7l3 3M14 14l3 3M7 17l3-3M14 10l3-3"
          opacity="0.6"
        />
      </svg>
    </div>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`text-xs font-medium ${isUser ? "text-cyan-100" : "text-gray-500 dark:text-gray-400"}`}
          >
            {isUser ? "YOU" : "AGENT"}
          </span>
          {message.model && (
            <span
              className={`text-xs ${isUser ? "text-cyan-200/70" : "text-gray-400 dark:text-gray-500"}`}
            >
              · {message.model}
            </span>
          )}
        ...
        {message.transcript && (
          <div
            className={`mb-3 rounded-xl px-3 py-2 text-xs ${isUser ? "bg-cyan-400/20 text-cyan-50" : "bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200"}`}
          >
            ...
            {(message.transcript.provider || message.transcript.language) && (
              <div
                className={`mt-1 ${isUser ? "text-cyan-100/80" : "text-gray-500 dark:text-gray-400"}`}
              >
                {[message.transcript.provider, message.transcript.language]
                  .filter(Boolean)
                  .join(" · ")}
        ...
        {message.voiceState && (
          ...
          >
            ...
            {message.voiceState.durationMs && (
              <span
                className={
                  isUser ? "text-cyan-100/80" : "text-sky-600 dark:text-sky-400"
                }
              >
                ...
                if (segment.type === "text") {
                  return (
                    <Fragment key={`text-${index}`}>
                      {renderAssistantText(segment.content, `text-${index}`)}
                    </Fragment>
                  );
                }
        ...
        {audioUrl && (
          <div className="mt-3">
            ...
            <div
              className={`mt-1 text-xs ${isUser ? "text-cyan-100/70" : "text-gray-500 dark:text-gray-400"}`}
            >
              {[
        ...
        {/* 时间戳 */}
        <div
          className={`text-xs mt-2 ${isUser ? "text-cyan-100/60" : "text-gray-400 dark:text-gray-500"}`}
        >
